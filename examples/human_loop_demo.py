...
async def example_basic_approval():
    ...
    strategies = [RiskBasedApprovalStrategy(
...
async def example_keyword_detection():
    ...
    # 创建基于关键词的策略
    interaction_handler = CLIInteractionHandler(console)
    strategies = [KeywordApprovalStrategy(
        sensitive_keywords=["删除", "清空", "购买", "出售"]
    )]
...
async def example_final_answer_approval():
    ...
    if approval_response:
        ...
        if approval_response.modified_answer:
            console.print(f"[blue]修改后的答案: {approval_response.modified_answer}[/blue]")
    else:
...
async def example_integrated_with_kernel():
    ...
    try:
        ...
        # 创建测试任务
        task = Task(
            description="测试Human-in-the-Loop功能",
            max_iterations=3,
            timeout=60
        )
