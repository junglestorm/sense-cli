...
def create_custom_approval_strategy():
    ...
    class CustomFinancialApprovalStrategy(HumanApprovalStrategy):
        ...
        def should_request_approval(
            ...
            # 1. 检查是否涉及金融交易
            if context.action and any(
                keyword in context.action.lower()
                for keyword in ["trade", "buy", "sell", "transfer", "payment"]
            ):
            ...
            if context.action_input:
                for key, value in context.action_input.items():
                    if key.lower() in ["amount", "quantity", "price"] and isinstance(
                        value, (int, float)
                    ):
                        if value > 10000:  # 大额交易
                            return (
