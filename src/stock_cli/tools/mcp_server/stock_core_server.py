...
@mcp.tool()
def get_stock_history(symbol: str, period: str = "daily", days: int = 30) -> Dict[str, Any]:
    """
    获取股票历史K线数据
    ...
    try:
        ...
        for _, row in matches.iterrows():
            results.append({
                "name": row['名称'],
                "code": row['代码'],
                "current_price": float(row['最新价']) if '最新价' in row and row['最新价'] != '-' else 0.0,
                "change_percent": float(row['涨跌幅']) if '涨跌幅' in row and row['涨跌幅'] != '-' else 0.0
            })
...
def get_company_profile(symbol: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # 使用雪球数据获取公司基本信息
        df = ak.stock_individual_basic_info_xq(symbol=f"SH{symbol}" if symbol.startswith('6') else f"SZ{symbol}")
