...
# 完全禁用MCP相关的控制台日志输出
for name in ["mcp", "mcp.server", "mcp.server.fastmcp", "mcp.server.lowlevel", "__main__"]:
...
def get_stock_news(symbol: str, limit: int = 10) -> Dict[str, Any]:
    ...
    try:
        ...
        for _, row in news_df.head(limit).iterrows():
            news_list.append({
                "title": row['新闻标题'],
                "content": row['新闻内容'][:200] + "..." if len(row['新闻内容']) > 200 else row['新闻内容'],
    ...
    try:
        ...
        try:
            ...
            sentiment_data = {
                ...
                "rising_ratio": round(rising_count / total_count * 100, 2) if total_count > 0 else 0,
                "falling_ratio": round(falling_count / total_count * 100, 2) if total_count > 0 else 0,
                "market_sentiment": "积极" if rising_count > falling_count else "谨慎" if rising_count < falling_count else "平衡",
                "timestamp": datetime.now().isoformat()
            }
...
@mcp.tool()
def calculate_technical_indicators(symbol: str, indicator: str = "ma") -> Dict[str, Any]:
    """
    计算技术指标
    ...
    try:
        ...
        if indicator == "ma":
            ...
            result.update({
                ...
                "trend_analysis": {
                    "short_trend": "上升" if latest['收盘'] > latest['MA5'] else "下降",
                    "medium_trend": "上升" if latest['MA5'] > latest['MA10'] else "下降",
                    "long_trend": "上升" if latest['MA10'] > latest['MA20'] else "下降"
                }
            })
            ...
            result.update({
                ...
                "rsi_signal": "超买" if latest_rsi > 70 else "超卖" if latest_rsi < 30 else "正常"
            })
    ...
    try:
        ...
        for _, row in df_sorted.iterrows():
            hot_stocks.append({
                "symbol": row['代码'],
                "name": row['名称'],
                "current_price": float(row['最新价']) if row['最新价'] != '-' else 0.0,
                "change_percent": float(row['涨跌幅']) if row['涨跌幅'] != '-' else 0.0,
                "change_amount": float(row['涨跌额']) if row['涨跌额'] != '-' else 0.0,
                "turnover": float(row['成交额']) if row['成交额'] != '-' else 0.0,
                "volume": int(row['成交量']) if row['成交量'] != '-' else 0,
                "turnover_rate": float(row['换手率']) if '换手率' in row and row['换手率'] != '-' else 0.0
            })
        ...
        analysis = {
            ...
            "avg_change_percent": round(sum([s['change_percent'] for s in hot_stocks]) / len(hot_stocks), 2),
