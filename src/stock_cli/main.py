...
@app.command()
def rich(
    loop_interval: int = typer.Option(30, "--interval", help="循环检查间隔秒"),
    dry_run: bool = typer.Option(False, "--dry-run", help="仅打印将要执行的任务不真正运行"),
):
    """i am rich 模式: 永不停歇执行触发器 & 事件驱动任务 (实验功能)"""
    _setup_logging("ERROR")
    if not _check_llm_config():
        console.print("[red]LLM配置不完整，请配置 config/settings.yaml[/red]")
        raise typer.Exit(1)

    async def _rich_loop():
        kernel = await ensure_kernel()
        console.print("[bold green]🏁 Rich 模式启动: 无限触发器循环[/bold green]")
        config_path = Path("config/triggers.yaml")
        last_reload = 0.0
        scheduled_cache = []  # (TriggerEvent, next_run_datetime)

        def _load_triggers():
            nonlocal scheduled_cache, last_reload
            try:
                stat = config_path.stat()
                if stat.st_mtime <= last_reload and scheduled_cache:
                    return
                ...
                for item in data.get("scheduled_tasks", []):
                    ...
                    if not cron_expr:
                        continue
                    trig = TriggerEvent(
                        name=item.get("name","scheduled_task"),
                        type="time",
                        condition={"cron": cron_expr},
                        task_template=item.get("task_template","执行任务"),
                    ...
                    try:
                        ...
                        continue
                    scheduled_cache.append((trig, nxt))
                last_reload = stat.st_mtime
            except Exception as e:
                console.print(f"[red]加载触发器失败: {e}")

        async def _run_task(desc: str, priority: TaskPriority):
            if dry_run:
                console.print(f"[yellow][DRY] 任务: {desc}[/yellow]")
                return
            from .core.types import Task
            task = Task(description=desc, priority=priority)
            try:
                await kernel.execute_task(task, stream=True)
                console.print(f"[green]✅ 完成: {desc[:40]}...[/green]")
            except Exception as e:  # noqa: BLE001
        ...
        try:
            while True:
                ...
                to_remove = []
                for idx,(trig,nxt) in enumerate(scheduled_cache):
                    if now >= nxt:
                        console.print(f"[cyan]⏰ 触发: {trig.name} ({nxt.astimezone().strftime('%Y-%m-%d %H:%M:%S')})[/cyan]")
                        await _run_task(trig.task_template, trig.priority)
                        try:
                            itr = croniter(trig.condition.get("cron"), now)
                            scheduled_cache[idx] = (trig, itr.get_next(datetime))
                        except Exception:
                            to_remove.append(idx)
                # 清理失效
                if to_remove:
                    scheduled_cache = [v for i,v in enumerate(scheduled_cache) if i not in to_remove]
                await asyncio.sleep(loop_interval)
        except KeyboardInterrupt:
            console.print("\n[yellow]停止 Rich 模式[/yellow]")

    asyncio.run(_rich_loop())
