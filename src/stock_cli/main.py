...
def main_callback(
    ...
    if ctx.invoked_subcommand is None:
        ...
        asyncio.run(_interactive(None, None, False, True, False, False, False))  # 默认启用 verbose
...
def _setup_logging(level: str = "INFO"):
    ...
    # 禁用所有可能产生控制台输出的日志
    noisy_loggers = [
        "posthog", "httpx", "httpcore", "openai", "mcp", "anyio", "asyncio",
        "src.tools.mcp_server_manager", "mcp.server.lowlevel.server", "__main__",
        "src.tools.mcp_server.stock_core_server", "src.tools.mcp_server.stock_news_server",
...
def _check_llm_config() -> bool:
    ...
    try:
        ...
        # 检查是否有任何有效的提供商配置
        valid_providers = ["openai", "deepseek", "ollama", "gemini", "anthropic", "azure", "custom"]
...
def _show_logo():
    ...
    console.print("[green]AI-Powered Stock Analysis Tool powered by ReAct Architecture[/green]\n")
    ...
    *,
    ...
    async def on_progress(chunk: str):
        ...
        # 过滤掉原始的ReAct关键词
            ...
            pass  # 忽略这些原始关键词
        if capture_steps and (chunk.startswith("[Thought]") or chunk.startswith("[Action]")):
    ...
    try:
        ...
        if use_persistent_context and answer:
            ...
            if len(_persistent_context["conversation_history"]) > max_history_pairs * 2:
                # 保留最新的对话，删除最旧的
                _persistent_context["conversation_history"] = _persistent_context["conversation_history"][-(max_history_pairs * 2):]
...
def ask(
    ...
    output: str = typer.Option("text", "--output", "-o", help="输出格式: text|json|yaml"),
    ...
    human_approval: bool = typer.Option(False, "--human-approval", help="启用人工审批模式"),
    ...
    asyncio.run(_interactive(
        model, question, False, True, debug, human_approval, confirm,  # quiet=False, verbose=True
        output_format=output, timeout=timeout
    ))
