...
class XMLStreamFilter:
    ...
    def process_chunk(self, chunk: str) -> Tuple[str, Optional[str]]:
        ...
        for char in chunk:
            ...
            if self.in_tag:
                ...
                if char == '>':
                    ...
                    if self._is_opening_tag(self.current_tag):
                        ...
                        # 结束标签
                        if (tag_name == "thought" and self.state == FilterState.IN_THOUGHT or
                            tag_name == "action" and self.state == FilterState.IN_ACTION or
                            tag_name == "final_answer" and self.state == FilterState.IN_FINAL):
            ...
            # 不在标签中，根据当前状态决定是否输出
            if self.state in [FilterState.IN_THOUGHT, FilterState.IN_ACTION, FilterState.IN_FINAL]:
                result += char
                if not section_type:
