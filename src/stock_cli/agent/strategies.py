...
class ExecutionStrategies:
    ...
    def handle_consecutive_failures(
        self, consecutive_failures: int, max_iterations: int, current_iteration: int
    ) -> Optional[str]:
        """处理连续失败 - 简化逻辑"""
        if (
            ...
            return (
                "Observation: 连续工具调用失败，请检查工具名称和参数格式，"
                "或者如果已有足够信息则直接给出最终答案。"
            )
        return None

    def handle_short_task_fallback(
        self,
        ...
        if (
            iteration == 1
            and not has_action
            and len(task_description) < 50
            and len(scratchpad) <= 1
        ):
            return thought or "任务完成"
        return None

    def handle_context_summary(
...
class SimpleSummaryStrategy:
    ...
    def apply_summary(
        self, scratchpad: List[str], summary: str, recent_steps: List[str]
    ) -> List[str]:
        """应用总结结果"""
