...
from ..core.types import AgentConfig, Task, TaskStatus, LLMProviderConfig
...
from .human_loop import HumanLoopManager, ApprovalContext, ApprovalResult
from .xml_filter import XMLStreamFilter
...
class AgentKernel:
    ...
    def _check_completion_from_response(self, response: str) -> bool:
        ...
        return "<final_answer>" in response.lower() and "</final_answer>" in response.lower()
    ...
    def _extract_action_from_response(self, response: str) -> tuple[str, str]:
        ...
        if action_match:
            ...
            if '(' in action_content and ')' in action_content:
                ...
                params_str = action_content[action_content.find('(') + 1:action_content.rfind(')')]
    ...
    async def _execute_react_loop(
        ...
        *,
        ...
        while iteration < max_iterations:
            ...
            try:
                # 发射迭代开始事件
                await event_adapter.emit(ReActEvent(
                    ReActEventType.ITERATION_START,
                    {"iteration": iteration}
                ))

                # 1. 构建prompt并调用LLM（仅流式输出，无解析）
                response = await self._call_llm_with_stream(
                    task, context, iteration, total_start, total_timeout, 
                    stream, event_adapter
                )
                ...
                if action_name:
                    # 执行工具调用
                    tool_result = await self.tool_executor.execute(action_name, action_input)
                    ...
                    task.scratchpad.append(f"Iteration {iteration}: {action_name}({action_input}) -> {observation}")
                else:
                    # 没有工具调用，可能是思考或无效响应
                    consecutive_invalid += 1
    ...
    async def _call_llm_with_stream(
        ...
        total_start: float, total_timeout: float, stream: bool,
        event_adapter: ProgressCallbackAdapter
    ) -> str:
        """调用LLM，支持流式输出"""
        # 获取可用工具
        ...
        if stream:
            ...
            return await self._regular_llm_call(
                messages, dyn_tokens, iter_timeout
            )
            ...
            async for chunk in self.llm_provider.generate_stream(
                ...
                if chunk:
                    ...
                    if filtered_content.strip():
                        await event_adapter.emit(ReActEvent(
                            ReActEventType.STREAM_CHUNK,
                            {"content": filtered_content, "type": section_type or "default"}
                        ))
    ...
    def _handle_tool_result(
        self, step_record: str, action: str, action_input: Dict[str, Any],
        tool_result, task: Task, iteration: int
    ) -> str:
        """处理工具执行结果"""
    ...
    async def _synthesize_results(
        ...
        try:
            ...
            # 合并所有结果
            collected_info = "\n\n---\n\n".join(
                [f"分析结果 {i+1}:\n{result}" for i, result in enumerate(results) if result]
            )
