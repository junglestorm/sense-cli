...
class ResponseParser:
    ...
    def parse(self, response: str) -> ParsedStep:
        ...
        if not (thought or action or final_answer):
            thought, action, action_input, final_answer = self._try_simple_parse(response)
        
        # 最后的兜底：直接使用响应内容
        if not (thought or action or final_answer):
            if len(response) > 10:
                # 如果看起来像最终答案，就当作最终答案
                if any(keyword in response for keyword in ["综上", "总结", "总体来看", "结论", "因此", "所以"]):
    ...
    def _parse_action_simple(self, action_content: str) -> tuple[Optional[str], Dict[str, Any]]:
        """简化的动作解析"""
