...
from .events import ReActEvent, ReActEventType
...
class RiskBasedApprovalStrategy(HumanApprovalStrategy):
    ...
    def __init__(self, 
        ...
        self.high_risk_tools = high_risk_tools or [
            "execute_trade", "send_email", "file_write", "system_command"
        ]
    ...
    def should_request_approval(self, context: ApprovalContext) -> tuple[bool, ApprovalType, str]:
        ...
        # 3. 检查点间隔
        if (self.checkpoint_intervals > 0 and 
            context.iteration > 0 and 
            context.iteration % self.checkpoint_intervals == 0):
            return True, ApprovalType.ITERATION_CHECKPOINT, f"第{context.iteration}轮检查点"
    ...
    def should_request_approval(self, context: ApprovalContext) -> tuple[bool, ApprovalType, str]:
        ...
        for keyword in self.sensitive_keywords:
            if keyword.lower() in full_text:
                return True, ApprovalType.HIGH_RISK_ACTION, f"检测到敏感关键词: {keyword}"
...
class CLIInteractionHandler(InteractionHandler):
    ...
    async def request_approval(self, request: ApprovalRequest) -> ApprovalResponse:
        ...
        try:
            ...
            try:
                ...
                self.console.print(f"[yellow]审批请求超时({request.timeout_seconds}s)，自动拒绝[/yellow]")
    ...
    def _get_user_input(self, request: ApprovalRequest) -> ApprovalResponse:
        ...
        while True:
            ...
            try:
                ...
                return ApprovalResponse(result=ApprovalResult.CANCELLED, message="用户中断")
            
            if user_input in ['y', 'yes', '是', 'ok']:
                return ApprovalResponse(result=ApprovalResult.APPROVED, message="用户批准")
            
            elif user_input in ['n', 'no', '否', 'reject']:
                return ApprovalResponse(result=ApprovalResult.REJECTED, message="用户拒绝")
            
            elif user_input in ['c', 'cancel', '取消']:
                return ApprovalResponse(result=ApprovalResult.CANCELLED, message="用户取消")
    ...
    def _handle_modification(self, request: ApprovalRequest) -> ApprovalResponse:
        ...
        try:
            ...
            if not modification:
                return ApprovalResponse(result=ApprovalResult.REJECTED, message="修改内容为空")
            ...
            return ApprovalResponse(result=ApprovalResult.CANCELLED, message="修改被中断")
...
class HumanLoopManager:
    ...
    async def check_approval_needed(self, context: ApprovalContext) -> Optional[ApprovalRequest]:
        """检查是否需要人工审批"""
        if not self.enabled:
            return None
        
        for strategy in self.strategies:
            needs_approval, approval_type, reason = strategy.should_request_approval(context)
            if needs_approval:
                ...
                return ApprovalRequest(
                    ...
                    context=context,
                    message=reason,
                    allow_modification=(approval_type != ApprovalType.ITERATION_CHECKPOINT)
                )
        
        return None
    
    async def request_approval_if_needed(self, context: ApprovalContext) -> Optional[ApprovalResponse]:
        """如果需要则请求人工审批"""
...
# 便利函数：创建默认的HITL配置
def create_default_human_loop(console=None, 
                             high_risk_tools: List[str] = None,
                             require_final_approval: bool = False) -> HumanLoopManager:
